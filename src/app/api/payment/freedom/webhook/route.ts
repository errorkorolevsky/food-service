...
import { verifyFreedomWebhook } from "@/lib/payment"
...
export async function POST(req: NextRequest) {
  ...
  const signature = req.headers.get("x-freedom-signature") ?? req.headers.get("x-pg-signature") ?? ""

  if (!process.env.FREEDOM_SECRET_KEY) {
    return new Response("Not configured", { status: 501 })
  }

  const result = verifyFreedomWebhook(body, signature)
  if (!result) {
